...
Node* searchNode(Node* root, int data) {
	...
	if (data < root->data) return searchNode(root->leftChild, data);
...
Node* deleteNode(Node* root, int data) {
	...
	if (data < root->data) root->leftChild = deleteNode(root->leftChild, data);
	else if (data > root->data) root->rightChild = deleteNode(root->rightChild, data);
	else if (root->leftChild != NULL && root->rightChild != NULL) {
		Node* node = findMinNode(root->rightChild);
		root->data = node->data;
